...
use App\Application\ShadowPresence\ContextHub;
use App\Application\ShadowPresence\ConversationBridge;
use App\Application\ShadowSecondBrain\DuplicateConceptResolver;
use App\Application\ShadowSecondBrain\KnowledgeAggregator;
use App\Application\ShadowSecondBrain\KnowledgeMergeEngine;
use App\Application\ShadowSecondBrain\KnowledgeStatisticsBuilder;
...
use App\Infrastructure\ShadowSecondBrain\InMemoryShadowSecondBrainRepository;
use App\Infrastructure\ShadowTeaching\InMemoryShadowTeachingRepository;
use PHPUnit\Framework\TestCase;

final class ContextHubTest extends TestCase
{
    public function testBuildAggregatesContextFromExistingEngines(): void
    {
        $identityRepository = new InMemoryShadowIdentityRepository();
        ...
        $workspaceBuilder = $this->workspaceBuilder($executiveCoordinator, $knowledgeBuilder, $memoryBuilder, $mentorBuilder, $teachingBuilder);
        ...
        $hub = new ContextHub(
            ...
            $knowledgeBuilder,
            $identityRepository,
            new ConversationBridge(new InMemoryShadowSessionRepository()),
        );

        $context = $hub->build('default', PresenceSurface::Web);

        self::assertSame('default', $context->scopeKey());
        self::assertSame(PresenceSurface::Web, $context->surface());
        self::assertSame('teacher', $context->identityLabel());
        self::assertGreaterThanOrEqual(2, $context->conceptCount());
        self::assertContains('identity:persona', $context->explainability());
        self::assertContains('knowledge:graph', $context->explainability());
    }
    ...
    private function workspaceBuilder(
        ...
        $brainRepository = new InMemoryShadowSecondBrainRepository();
        ...
        return new WorkspaceBuilder(
            ...
            $executiveCoordinator,
            $aggregator,
            new DuplicateConceptResolver(),
            new KnowledgeMergeEngine(),
            new KnowledgeStatisticsBuilder(),
        );
    }
}
