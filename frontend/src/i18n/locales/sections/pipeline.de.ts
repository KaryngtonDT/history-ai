...
export const pipelineDe: PipelineMessages = {
	pipeline: {
		...
		progress: {
			...
			remainingTime: "~{{time}} verbleibend (geschätzt)",
			estimatedCompletion: "Geschätzte Fertigstellung: {{time}}",
			actualCompletion: "Abgeschlossen um {{time}}",
			actualDuration: "Tatsächliche Dauer: {{time}}",
			estimationAccuracy: "Vorhersagegenauigkeit: {{percent}}",
			elapsedTime: "Verstrichen: {{time}}",
			engine: "Engine: {{engine}}",
			engineVersion: "Engine-Version: {{version}}",
			provider: "Anbieter: {{provider}}",
			hardwareProfile: "Hardware-Profil: {{profile}}",
			currentStep: "Stufe: {{step}}",
			checkpoint: "Checkpoint: {{checkpoint}}",
			processingSpeed: "Geschwindigkeit: {{speed}}",
			currentSegment: "Aktuelles Segment: {{current}} / {{total}}",
			audioProcessed: "Audio verarbeitet: {{processed}} / {{total}}",
			worker: "Worker: {{worker}}",
			dockerContainer: "Docker-Container: {{container}}",
			waitingForWorker: "Warte auf Worker-Aktualisierung…",
			averageSpeed: "Durchschnittsgeschwindigkeit: {{speed}}",
		},
		create: {
