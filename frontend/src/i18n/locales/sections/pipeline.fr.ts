...
export const pipelineFr: PipelineMessages = {
	pipeline: {
		...
		progress: {
			...
			remainingTime: "~{{time}} restantes (estimé)",
			estimatedCompletion: "Fin estimée : {{time}}",
			actualCompletion: "Terminé à {{time}}",
			actualDuration: "Durée réelle : {{time}}",
			estimationAccuracy: "Précision de prédiction : {{percent}}",
			elapsedTime: "Écoulé : {{time}}",
			engine: "Moteur : {{engine}}",
			engineVersion: "Version moteur : {{version}}",
			provider: "Fournisseur : {{provider}}",
			hardwareProfile: "Profil matériel : {{profile}}",
			currentStep: "Étape : {{step}}",
			checkpoint: "Point de contrôle : {{checkpoint}}",
			processingSpeed: "Vitesse : {{speed}}",
			currentSegment: "Segment actuel : {{current}} / {{total}}",
			audioProcessed: "Audio traité : {{processed}} / {{total}}",
			worker: "Worker : {{worker}}",
			dockerContainer: "Conteneur Docker : {{container}}",
			waitingForWorker: "En attente de mise à jour du worker…",
			averageSpeed: "Vitesse moyenne : {{speed}}",
		},
		create: {
			videoEyebrow: "Créer",
